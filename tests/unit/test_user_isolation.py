"""User isolation is a code invariant. These tests are what enforce it.

The cluster runs the Basic licence, so there is no document-level security to
fall back on. If a query leaves `app.search.query` without a user constraint,
...
def _user_uuid_terms(query: dict[str, Any]) -> list[Any]:
    """Every `user.uuid` constraint anywhere in the query tree."""
    found: list[Any] = []
...
def _all_filter_permutations() -> list[AuditSearchFilter]:
    ...
    single filter, and no interaction between two, can displace the user
    clause.
    """
...
def test_user_filter_present_for_every_filter_permutation(
    criteria: AuditSearchFilter,
) -> None:
    """No caller-supplied filter can remove or override the user constraint."""
    scope = UserScope(user_uuid="user-a")
    query = build_query(scope, criteria, max_window_days=WINDOW_DAYS, now=NOW)

    terms = _user_uuid_terms(query)
    assert terms == ["user-a"], (
        f"expected exactly one user.uuid constraint of 'user-a', got {terms!r}. "
        "A missing or duplicated user clause is a cross-user data leak."
    )
    ...
    # user constraint bypassable.
    assert any(
        clause.get("term", {}).get("user.uuid") == "user-a" for clause in query["bool"]["filter"]
    ), "user constraint must be a top-level filter clause"


def test_user_filter_is_first_clause() -> None:
    """The user clause leads the filter list.
    ...
    assert query["bool"]["filter"][0] == {"term": {"user.uuid": "user-a"}}
...
# Scope construction
# ---------------------------------------------------------------------------
def test_user_scope_rejects_missing_user() -> None:
    """A scope with neither a user nor cross-user authority is unbuildable.
    ...
    """
    with pytest.raises(QueryValidationError, match="requires user_uuid"):
        UserScope(user_uuid=None)


def test_cross_user_scope_omits_the_filter_deliberately() -> None:
    """Cross-user access drops the constraint - only via the explicit flag."""
    query = build_query(
        UserScope(user_uuid=None, cross_user=True),
        AuditSearchFilter(),
        max_window_days=WINDOW_DAYS,
        now=NOW,
    )
    assert _user_uuid_terms(query) == []
...
def test_self_restriction_cannot_be_widened_by_criteria() -> None:
    ...
    """
    scope = UserScope(user_uuid="user-a", actor_id="me")
    query = build_query(
        scope,
...
def test_issuer_scope_is_applied_and_not_overridable() -> None:
    ...
    scope = UserScope(user_uuid="user-a", issuer_id="issuer-1")
    query = build_query(
        scope,
    ...
    clauses = query["bool"]["filter"]
    assert {"term": {"user.issuer_id": "issuer-1"}} in clauses
    # The caller's attempt to name a different issuer is not added as a second,
    # widening clause.
    assert {"term": {"user.issuer_id": "issuer-2"}} not in clauses


# ---------------------------------------------------------------------------
# User id validation - index-name injection
# ---------------------------------------------------------------------------
@pytest.mark.parametrize(
    ...
    [
        ...
        "../../audit-shared",
        "user/../other",
        "user,other",  # comma would target two indices
        "*",  # wildcard would match every index
        "audit-*",
        "user a",  # space is illegal in an index name
        "user#1",
        "user:1",
        'user"1',
        "user\\1",
...
def test_user_uuid_validation_rejects_hostile_values(hostile: str) -> None:
    """User ids are interpolated into index names, so they are validated.

    A user id arrives from the `x-audit-user-uuid` header. Unvalidated, a
    comma or a wildcard would let a caller widen a query to other users'
    indices - the index-name equivalent of SQL injection.
    """
    with pytest.raises(InvalidUserUuidError):
        UserRouter.validate_user_uuid(hostile)


@pytest.mark.parametrize(
    "acceptable",
    ["user-a", "7f3c9e12-4b8a-4c1d-9f2e-1a2b3c4d5e6f", "T1", "a.b-c_d", "9"],
)
def test_user_uuid_validation_accepts_real_identifiers(acceptable: str) -> None:
    assert UserRouter.validate_user_uuid(acceptable) == acceptable


# ---------------------------------------------------------------------------
# Routing
# ---------------------------------------------------------------------------
def test_shared_user_routes_to_shared_stream_with_routing_key(
    router: UserRouter,
) -> None:
    """A shared-stream user gets a routing key, pinning it to one shard."""
    decision = router.resolve("small-user")
    assert decision.dedicated is False
    assert decision.write_target == "audit-shared"
    assert decision.read_targets == ("audit-shared",)
    assert decision.routing_key == "small-user"


def test_dedicated_user_reads_both_streams(router: UserRouter) -> None:
    """A promoted user still sees history written before the promotion.

    Reads must cover the dedicated stream *and* the shared one, or promoting a
    user would appear to erase its past.
    """
    decision = router.resolve("big-user")
    assert decision.dedicated is True
    assert decision.write_target == "audit-u-big-user"
    assert decision.read_targets == ("audit-u-big-user", "audit-shared")
    # No routing key: the stream is already user-scoped, so pinning to one
    # shard would only remove headroom.
    assert decision.routing_key is None


def test_dedicated_and_shared_patterns_are_disjoint(router: UserRouter) -> None:
...
def test_partition_assignment_is_stable_across_processes(router: UserRouter) -> None:
    """Partition choice must not depend on Python's per-process hash seed.

    The hash chain is per (user, partition). If two workers disagreed about a
    user's partition, its sequence numbers would interleave across two chains
    and verification would report false breaks forever.
    """
    first = router.partition_for("user-a", 8)
    assert first == router.partition_for("user-a", 8)
    assert 0 <= first < 8
    ...
    assert router.partition_for("user-a", 8) == router.partition_for("user-a", 8)


def test_partition_distribution_is_reasonable(router: UserRouter) -> None:
    """Users spread across partitions rather than piling onto one."""
    counts = [0] * 8
    for index in range(400):
        counts[router.partition_for(f"user-{index}", 8)] += 1
    # With 400 users over 8 partitions the mean is 50; a partition holding
