...
compare, would hand over every tenant's audit trail.
"""
...
def test_service_principal_carries_every_scope(authenticator: Authenticator) -> None:
    ...
    Erase, admin and cross-tenant were previously reachable only through a
    scoped user token. With that path gone, withholding them here would leave
    erasure, tenant dedication and cross-tenant search permanently 403.
    """
...
def test_service_principal_takes_its_tenant_from_the_header(
    authenticator: Authenticator,
) -> None:
    """The tenant is per-request, not bound to the key."""
    principal = authenticator.service_principal(
        service_name="everycred-backend", tenant_id="tenant-a", on_behalf_of=None
    )
    assert principal.tenant_id == "tenant-a"

    unscoped = authenticator.service_principal(
        service_name="everycred-backend", tenant_id=None, on_behalf_of=None
    )
